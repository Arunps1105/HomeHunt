...
const UserChat = () => {
  ...
  const userId = role === "owner"
    ? Number(sessionStorage.getItem("oid"))
  ...
  useEffect(() => {
    axios.get(`${BASE}/Requested/?request_id=${id}`).then((res) => {
      ...
      if (role === "owner") {
        ...
        setPeerName(req.house_id__owner__owner_name   || "");
        setPeerPhoto(req.house_id__owner__owner_photo || "");
      }
    });
  ...
  const renderMessage = (msg, i) => {
    ...
    return (
      <div key={msg.id || i} className={`${styles.row} ${isMe ? styles.mine : styles.theirs}`}
        ...
        {!isMe && (peerPhoto
          ? <img src={`${BASE}/${peerPhoto}`} className={styles.avatar} alt="peer" onError={(e) => { e.target.style.display = "none"; }} />
    ...
    <div className={styles.chatWrapper}>
      <div className={styles.chatContainer}>
        <div className={styles.chatHeader}>
          {peerPhoto ? <img src={`${BASE}/${peerPhoto}`} className={styles.hAvatarImg} alt="peer" onError={(e) => { e.target.style.display = "none"; }} /> : <div className={styles.hAvatar}>{initials(peerName)}</div>}
        ...
        <div className={styles.chatMessages} ref={messagesRef} onScroll={handleScroll}>
          ...
          {isTyping && (
            <div className={styles.typingRow}>
              {peerPhoto ? <img src={`${BASE}/${peerPhoto}`} className={styles.avatar} alt="peer" /> : <div className={styles.av}>{initials(peerName)}</div>}
     ...
     {ctxMenu && <CtxMenu x={ctxMenu.x} y={ctxMenu.y} isMe={String(ctxMenu.msg?.sender_id) === String(userId)} onCopy={copyText} onDelete={deleteMsg} onClose={() => setCtxMenu(null)} />}
    </div>
  );
};
